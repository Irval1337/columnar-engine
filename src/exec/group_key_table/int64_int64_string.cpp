...
public:
    ...
    void Consume(const std::vector<const core::Column*>& key_cols,
        ...
        VisitIntegerCol(*key_cols[0], [&](const auto& first_typed) {
            VisitIntegerCol(*key_cols[1], [&](const auto& second_typed) {
                ...
                const auto* dict = core::AsDictionaryString(key_cols[2]);
                const core::StringColumn* strings =
                    dict == nullptr ? &static_cast<const core::StringColumn&>(*key_cols[2])
                                    : nullptr;
    ...
    void AppendKeys(uint32_t group_id, core::Batch& out) const override {
        ...
        out.ColumnAt(2).AppendFromString(key.third);
    }
...
private:
    ...
    struct GroupHash {
        ...
        const Int64Int64StringKeyTable* table = nullptr;
    ...
    struct GroupEq {
        ...
        bool operator()(uint32_t lhs, const ProbeKey& rhs) const noexcept {
            ...
            return table->hashes_[lhs] == rhs.hash && key.first == rhs.first &&
                   key.second == rhs.second && key.third == rhs.third;
        }
    ...
    void InsertGroup(uint32_t group_id, const ProbeKey& key) {
        ...
        auto interned = arena_.Intern(key.third);
        hashes_.push_back(key.hash);
        keys_.push_back(Key{key.first, key.second, interned});
...
std::unique_ptr<GroupKeyTable> MakeInt64Int64StringKeyTable(util::StringArena& arena) {
    return std::make_unique<Int64Int64StringKeyTable>(arena);
}
}  // namespace columnar::exec
